...
export function getTextTemplateForField(fieldName) {
  const templatesForField = getTextTemplates(fieldName);
  const template = templatesForField[Math.floor(Math.random() * templatesForField.length)];
  return template;
}
